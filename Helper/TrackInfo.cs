...
namespace Webradio.Helper
{
    public class TrackInfo
    {
        ...
        public TrackInfo(string artist, string title)
        {
            ...
            if (FrontCover == "")
            {
                FrontCover = GetFrontCoverByCoverArtArchive(ReleaseId);
            }
        ...
        private string GetFrontCoverByCoverArtArchive(string releaseId)
        {
            ...
            foreach (var s in c.Images.Where(s => s.Front))
            {
                return s.Image;
            }
            return "";
        ...
        private string GetCover(string title, string artist)
        {
            ...
            { 
                ...
                foreach (var a in cover2.Albums.SelectMany(c => c.Artwork.Where(a => a.Uri.AbsoluteUri != string.Empty)))
                {
